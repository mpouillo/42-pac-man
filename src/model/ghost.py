...
class Ghost:
    def __init__(self, ghost_type: GhostType, speed: float) -> None:
        ...
        self._target_tile: Tuple[int, int] = (0, 0)     # (y, x)
        self._checkpoint: Tuple[float, float] = (0, 0)  # (y, x)
    ...
    def _calculate_target_tile(
        ...
        match self.type:
            case GhostType.PINK:
                ...
                pacman_pos = numpy.array((int(pacman.y), int(pacman.x)))
            ...
            case GhostType.BLUE:
                ...
                red_yx = numpy.array((red_pos[1], red_pos[0]))
                difference = numpy.subtract(red_yx, pink_target)
    ...
    def _move_towards_target(self, level: Level, delta_time: float) -> None:
        if self._checkpoint == (0.0, 0.0):
            self._checkpoint = (float(int(self.y)), float(int(self.x)))

        dy = self._checkpoint[0] - self.y
        dx = self._checkpoint[1] - self.x
        cp_distance = math.hypot(dy, dx)
    ...
    def _choose_next_checkpoint(self, level: Level) -> None:
        ...
        for d in directions:
            ...
            next_x = int(self._checkpoint[1]) + d.value[0]
            next_y = int(self._checkpoint[0]) + d.value[1]
            ...
            if (
                ...
                dist_sq = math.sqrt(
                    (next_x - self._target_tile[1]) ** 2
                    + (next_y - self._target_tile[0]) ** 2
                )

                if dist_sq < min_distance:
                    min_distance = dist_sq
                    best_direction = d
                    best_checkpoint = (float(next_y), float(next_x))
