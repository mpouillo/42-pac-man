...
class Ghost:
    """Represent a ghost enemy NPC with specialized AI and pathfinding."""

    def __init__(self, ghost_type: GhostType, level: Level) -> None:
        """Initialize the ghost entity with map positioning and settings."""
    ...
    def update(
        ...
        delta_time: float,
        pacman: PacmanData,
        red_ghost: GhostData | None
    ) -> None:
        """Execute step-based position changes and handle targets."""
    ...
    def _init_scatter_target(self) -> Coordinates:
        """Assign fixed coordinates for target zones based on personality."""
    ...
    def _compute_target(
        ...
        """Calculate coordinate navigation objectives based on AI state."""
        ...
        # Corner Pacman relative to Red's position
            ...
            vec_x = front_x - (round(red_ghost.x) if red_ghost else 0)
            vec_y = front_y - (round(red_ghost.y) if red_ghost else 0)
            self._target = (
    ...
    def _pathfind_to(self, destination: Coordinates) -> None:
        """Pop the next step from an active path or generate a new A* path."""
    ...
    def _pick_direction(
        ...
        """Select the optimal valid direction towards a target."""
    ...
    def _astar(
        ...
        """Perform an A* search algorithm between start and goal."""
        if start == goal:
            return [goal]
